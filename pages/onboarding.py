...
with tab2:
    ...
    with st.expander("🔍 ④ 「できる」ようになるまで進ませない「確認テスト＆個別指導」"):
        ...
        * **個別指導:** テストの後は、1対1の個別指導を行います。ここでは単にわからない問題を教えるだけでなく、 **「答えの丸暗記になっていないか」「正しい勉強法ができているか」「プロセスを理解しているか」** を口頭で細かくチェックし、本質的な理解度を確認します。
        """)
    ...
    * 🛑 **今の学力と志望校に大きなギャップがある（E判定の）生徒**
    ...
    with col_link2:
        st.link_button("👪 武田塾 保護者の方へ", "https://www.takeda.tv/parent/", use_container_width=True)
...
with tab4:
    st.header("🎓 受験知識・塾知識")
    st.markdown("""
    インサイドセールスとして保護者や生徒と対等に話すためには、最低限の「受験・塾の知識」が必要です。
    以下の動画を視聴して、他塾との違いや受験の基本構造を頭に入れましょう！
    """)
